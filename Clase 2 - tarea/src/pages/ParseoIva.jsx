...
function toCsvContent(sheetHeaders, sheetRows) {
...
export default function ParseoIva() {
  ...
  useEffect(() => {
    if (sheetRows.length === 0 || sheetHeaders.length === 0) return;
    const content = toCsvContent(sheetHeaders, sheetRows);
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      setSheetPreview(
        rows.slice(0, 6).map((r) => r.map((c) => String(c ?? '')).join(' | ')).join('\n') +
          (rows.length > 6 ? '\n...' : '')
      );
      setCanGenerate(true);
    };
  ...
  const handleGenerate = () => {
    let content = toCsvContent(sheetHeaders, sheetRows);
    if (!content) return;
